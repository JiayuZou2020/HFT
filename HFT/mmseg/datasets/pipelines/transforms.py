...
# results have the following keys: ['img_info', 'ann_info', 'seg_fields', 'img_prefix', 'seg_prefix', 'filename', 'ori_filename','img',\
# 'img_shape', 'ori_shape', 'pad_shape', 'scale_factor', 'img_norm_cfg', 'gt_semantic_seg', 'calib', 'scale', 'scale_idx', 'keep_ratio']
...
# nuscenes dataset has 14 classes
@PIPELINES.register_module()
class RandomFlipNuscenes(object):
    """Flip the image & seg.
...
# argoverse dataset has 8 classes
@PIPELINES.register_module()
class RandomFlipArgoverse(object):
    """Flip the image & seg.
...
# kittiraw, kittiodometry, and kittiobject have 2 classes (including background)
# kittiraw and kittiodometry is used in evaluation of road segmentation
# kittiobject is used in evaluation of vehicle segmentation
@PIPELINES.register_module()
class RandomFlipKitti(object):
    """Flip the image & seg.
...
@PIPELINES.register_module()
class RandomCropNuscenes(object):
    """Random crop the image & seg.
    ...
    def __init__(self, crop_size, cat_max_ratio=1., ignore_index=0):
        assert crop_size[0] > 0 and crop_size[1] > 0
        self.crop_size = crop_size
        self.cat_max_ratio = cat_max_ratio
        self.ignore_index = ignore_index
        ...
        if np.random.rand() < self.cat_max_ratio:
            ...
            for key in results.get('seg_fields', []):
                ...
                for i in range(15):
                    ...
                    seg_temp = self.crop(seg_temp, (int(crop_bbox[0]*196/600),int(crop_bbox[1]*196/600),\
                                                        int(crop_bbox[2]*200/800),int(crop_bbox[3]*200/800)))
...
@PIPELINES.register_module()
class RandomRotateNuscenes(object):
    """Rotate the image & seg.
...
# HideAndSeek is like cutout data augmentation
@PIPELINES.register_module()
